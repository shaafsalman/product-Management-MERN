...
const OfflineHome = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('/api/offlineProducts/');
  ...
  const handleSearch = () => {
    if (searchTerm === '') {
      ...
      // Ensure results is always an array before setting searchResults
  ...
  const handleDeleteProduct = async (productId) => {
    try {
      ...
      await axios.delete(`/api/offlineProducts/delete/${productId}`);
