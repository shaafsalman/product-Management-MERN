...
const AddProduct = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try 
    {
      console.log("adding send request to backend");
      const response = await axios.post('https://localhost:3000/api/products/add', { name, price });
      onAdd(response.data); 
      onClose(); 
    } 
    catch (error) {
      ...
      <div className="max-w-md w-full p-6 bg-gray-100 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="flex justify-end">
            ...
            <button type="submit" onClick={handleSubmit} className="px-4 py-2 bg-indigo-500 text-white rounded-md">Add</button>
          </div>
        </form>
      </div>
    </div>
